...
//В некоторых случаях мы не хотим, чтобы классы имели наследников, это может быть сделано для того, чтобы предотвратить ошибочное использование классов
//Например в классе нет виртуальных функций, наследование всё равно не даст полиморфизма (обратим внимание, что приватное наследование ПОЛНОСТЬЮ может быть заменено на композицию)
//Или мы создали дочерний класс и полагаем, что дальнейшее его расширение может быть опасно
//Для этого есть ключевое слово final - оно не даёт возможности наследоваться от такого класса

struct FinalStruct final {
	virtual std::string what() const { return "I'm final class"; }
};

/*struct FinalStructDerived: FinalStruct { //ошибка компиляции нельзя наследовать от класса с квалификатором final
...
struct FinalMethodStructDerived : FinalMethodStruct {
	//std::string what() const { return "I'm final method"; }//ошибка компиляции, невозможно переопределить, т.к. метод объявлен final
