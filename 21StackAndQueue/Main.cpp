...
//Третья важная безадресная структура - это очередь с приоритетом
//Основное отличие очереди с приоритетом, от обычной очереди в том, что элементы при выборе из очереди следуют не просто принципу FIFO, но следуют приоритету
//Рассмотрим случай: есть люди и мы хотим обслужить их по возрасту так, что самые старшие проходят первыми
//Они попадают в очередь в произвольном порядке {Anna,27}, {Bert,34}, {Uncle Bob, 57}, {Duke,40}, {Cory,27}
//Элементы будут взяты из очерели в следующем порядке: {Uncle Bob, 57} -> {Duke,40} -> {Bert,34} -> {Anna,27} -> {Cory,27}
//Порядок элементов определяется возрастом, елси возраст одинаков, то действует принцип FIFO

//Быстрая реализация такой очереди, когда на каждом шаге необходимо вынимать элемент, квалифицированный как максимальный по приоритету неочевидна, эффективный способ - использование бинарной кучи
//её не нужно реализовывать самому, в STL есть обёртка, которая сама выполняет все необходимые операции: priority_queue
#include <queue>
...
//priority_queue полагается на сравнение оператором <
